...
var ChromiumAdapter = class {
  ...
  async setAlarm(name, triggerTimeMs) {
...
var PREDEFINED_CATEGORIES = [
  {
    id: "social",
    name: "Social Media",
    defaultDomains: ["facebook.com", "twitter.com", "instagram.com", "tiktok.com", "reddit.com", "linkedin.com"]
  },
  {
    id: "video",
    name: "Video & Streaming",
    defaultDomains: ["youtube.com", "netflix.com", "twitch.tv", "vimeo.com", "hulu.com", "disneyplus.com"]
  },
  {
    id: "gaming",
    name: "Gaming",
    defaultDomains: ["roblox.com", "minecraft.net", "steamcommunity.com", "discord.com", "epicgames.com"]
  },
  {
    id: "shopping",
    name: "Shopping",
...
// blocking/rules-manager.ts
var BlockingRuleManager = class {
  adapter;
   ...
   */
  resolveDomains(categories, customDomains, profileEmail = "", studyEmail = "") {
    ...
    const hasActiveStudyEmail = profileEmail && studyEmail && profileEmail.trim().toLowerCase() === studyEmail.trim().toLowerCase();
   ...
   */
  async enableRules(categories, customDomains) {
...
// blocking/engine.ts
var CoreFocusEngine = class {
  adapter;
  rulesManager;
  alarmName = "focus_lock_unlock";
  isRestoring = false;
  constructor(adapter2) {
    this.adapter = adapter2;
    this.rulesManager = new BlockingRuleManager(adapter2);
  }
  async init() {
  ...
  async checkAndRestoreSession() {
    ...
    try {
      ...
      if (active) {
        ...
        if (remaining > 0) {
          console.log(`[Engine] Restoring active focus session. Remaining: ${Math.round(remaining / 1e3)}s`);
  ...
  async startFocusSession(durationMinutes, blockedCategories, customDomains, strictMode = false, sessionId = null, absoluteEndsAt = null) {
    const endsAt = absoluteEndsAt || Date.now() + durationMinutes * 60 * 1e3;
  ...
  async startBlocking(blockedCategories, customDomains, endsAt) {
...
// supabase/client.ts
var SUPABASE_URL = "https://rqkgavwtdtunsctjclpm.supabase.co";
var SUPABASE_ANON_KEY = "sb_publishable_o4DJ-nSwhjMZAd-pNJJiGw_hMHuLsJn";
...
var SyncManager = class {
  ...
  async syncLatestSession() {
    if (!this.currentUserId)
      return;
    try {
      const { data: settings } = await supabase.from("user_settings").select("study_email").eq("user_id", this.currentUserId).maybeSingle();
      if (settings && settings.study_email !== void 0) {
      ...
      const { data: session, error } = await supabase.from("focus_sessions").select(`
          id,
          status,
          ends_at,
          strict_mode,
        ...
        `).eq("user_id", this.currentUserId).eq("status", "active").order("started_at", { ascending: false }).limit(1).maybeSingle();
      if (error)
        throw error;
      if (session) {
  ...
  subscribeToRealtime(userId) {
    ...
    this.activeChannel = supabase.channel(`focus_sessions_sync_${userId}`).on(
      "postgres_changes",
      {
        event: "*",
        schema: "public",
      ...
      async (payload) => {
        ...
        const newRow = payload.new;
        if (newRow && newRow.study_email !== void 0) {
          ...
          if (localState.active) {
            await this.engine.startFocusSession(
              Math.round((localState.endsAt - Date.now()) / 6e4),
  ...
  async fetchAndApplySession(sessionId) {
    try {
      const { data: session, error } = await supabase.from("focus_sessions").select(`
          id,
          status,
  ...
  async applyRemoteSession(session) {
    ...
    const categories = session.focus_session_categories?.map((c) => c.category_id) || [];
    const customDomains = session.focus_session_custom_sites?.map((s) => s.domain) || [];
    if (session.status === "active" && !isExpired) {
      const localState = await this.engine.getSessionState();
      const isAlreadyMatching = localState.active && localState.sessionId === session.id && localState.strictMode === (session.strict_mode || false) && Math.abs(localState.endsAt - endsAtMs) < 2e3 && // allow small round-trip millisecond difference
      JSON.stringify(localState.blockedCategories.sort()) === JSON.stringify(categories.sort()) && JSON.stringify(localState.customDomains.sort()) === JSON.stringify(customDomains.sort());
      if (!isAlreadyMatching) {
        console.log(`[SyncManager] Applying remote session lock until: ${new Date(endsAtMs).toISOString()}`);
        const durationMinutes = Math.round((endsAtMs - Date.now()) / 6e4);
        await this.engine.startFocusSession(
      ...
      if (!localState.active || !localState.sessionId || localState.sessionId === session.id) {
        ...
        try {
          await this.engine.cancelFocusSession(true);
        } catch (err) {
...
// background/index.ts
var adapter = new ChromiumAdapter();
var engine = new CoreFocusEngine(adapter);
var syncManager = new SyncManager(engine);
engine.init().then(() => {
  syncManager.init();
}).catch((err) => {
...
adapter.onMessage((message, sendResponse) => {
  if (message.type === "START_SESSION") {
    ...
    engine.cancelFocusSession().then(() => sendResponse({ success: true })).catch((err) => sendResponse({ success: false, error: err.message }));
  } else if (message.type === "RESET_SESSION") {
    engine.resetFocusSession().then(() => sendResponse({ success: true })).catch((err) => sendResponse({ success: false, error: err.message }));
  } else if (message.type === "GET_SESSION") {
    engine.getSessionState().then((session) => sendResponse({ success: true, session })).catch((err) => sendResponse({ success: false, error: err.message }));
  } else if (message.type === "CHECK_YOUTUBE_SESSION") {
    engine.getSessionState().then(async (session) => {
        ...
        if (change.newValue) {
          try {
            ...
            supabase.auth.getSession().then(({ data: { session: currentSession } }) => {
              ...
              supabase.auth.setSession(sessionObj).then(({ error }) => {
                if (error) {
