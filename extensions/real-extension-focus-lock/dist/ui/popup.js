...
document.addEventListener("DOMContentLoaded", () => {
  ...
  stopBtn.addEventListener("click", () => {
    adapter.sendMessage({ type: "STOP_SESSION" }).then((response) => {
  ...
  restartBtn.addEventListener("click", () => {
    adapter.sendMessage({ type: "RESET_SESSION" }).then((response) => {
      if (response && response.success) {
        updateUI();
      } else {
  ...
  async function updateSyncInfo() {
    try {
      ...
      if (profileEmail && studyEmail && profileEmail.trim().toLowerCase() === studyEmail.trim().toLowerCase()) {
        exceptionStatusDisplay.textContent = "\u{1F7E2} ACTIVE (YouTube Allowed)";
        exceptionStatusDisplay.style.color = "#047857";
      } else {
        exceptionStatusDisplay.textContent = "\u{1F534} INACTIVE (YouTube Blocked)";
        exceptionStatusDisplay.style.color = "#C73A57";
      }
  ...
  function updateUI() {
    void updateSyncInfo();
    adapter.sendMessage({ type: "GET_SESSION" }).then((response) => {
      if (response && response.success && response.session) {
        ...
        if (isCompleted) {
          ...
          completedView.style.display = "none";
          const activeList = [...blockedCategories.map((c) => c.toUpperCase()), ...customDomains];
          if (strictMode) {
