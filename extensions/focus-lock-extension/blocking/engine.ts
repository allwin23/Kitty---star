...
export class CoreFocusEngine {
  ...
  async checkAndRestoreSession() {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          '[Engine] Focus Lock storage corrupted or incomplete. Resetting local session...',
        );
      ...
      if (active) {
        ...
        if (remaining > 0) {
          console.log(
            `[Engine] Restoring active focus session. Remaining: ${Math.round(remaining / 1000)}s`,
          );
          await this.startBlocking(blockedCategories, customDomains, endsAt);
        } else {
          console.log(
            '[Engine] Stored focus session already expired. Transitioning to complete...',
          );
  ...
  async startFocusSession(
    ...
    absoluteEndsAt: number | null = null,
  ) {
